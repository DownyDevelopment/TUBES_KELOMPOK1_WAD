...
<div class="min-h-screen bg-black text-white">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('dosen.create') }}" class="bg-yellow-400 text-black px-4 py-2 rounded-lg hover:bg-yellow-300">Tambah Dosen</a>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-800">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">NIDN</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Prodi</th>
                ...
                <tbody class="divide-y divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-800">
                        ...
                        <td class="px-6 py-4">{{ $dsn->nidn }}</td>
                        <td class="px-6 py-4">{{ $dsn->prodi }}</td>
                        <td class="px-6 py-4">{{ $dsn->alamat }}</td>
                        <td class="px-6 py-4">{{ $dsn->no_hp }}</td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('dosen.edit', $dsn->id) }}" class="bg-yellow-400 text-black px-2 py-1 rounded">Edit</a>
                            <form action="{{ route('dosen.destroy', $dsn->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
